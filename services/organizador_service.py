...
async def create_organizador(
    organizador: OrganizadorCreateSchema, db: AsyncSession
):
    novo_organizador: Organizador = Organizador(
        nome=organizador.nome,
        email=organizador.email,
        telefone=organizador.telefone,
        senha=generate_hashed_password(organizador.senha),
    )
    async with db as session:
        try:
            ...
            raise HTTPException(
                ...
                detail="Email informado já cadastrado.",
            )


async def get_organizador(organizador_id: UUID, db: AsyncSession):
    async with db as session:
        organizador = await pegar_organizador(
            organizador_id=organizador_id, db=session
        )

        if organizador is None:
            raise HTTPException(
                status.HTTP_404_NOT_FOUND, detail="Organizador não encontrado"
            )

        return organizador


async def update_organizador(
    organizador_id: UUID,
    organizador: OrganizadorUpdateSchema,
    db: AsyncSession,
):
    async with db as session:
        update_organizador = await pegar_organizador(
            organizador_id=organizador_id, db=session
        )
...
async def delete_organizador(organizador_id: UUID, db: AsyncSession):
    async with db as session:
        delete_organizador = await pegar_organizador(
            organizador_id=organizador_id, db=session
        )
...
async def login_organizador(login_data: LoginSchema, db: AsyncSession):
    async with db as session:
        query = select(Organizador).filter(
            Organizador.email == login_data.email
        )
