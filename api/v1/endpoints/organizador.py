...
@router.patch("/", response_model=OrganizadorSchema, status_code=status.HTTP_202_ACCEPTED, responses={**auth_responses, **generate_not_found_response("Organizador")})
async def patch(organizador: OrganizadorUpdateSchema, db: AsyncSession = Depends(get_session), usuario_logado: Organizador = Depends(get_current_user)):
