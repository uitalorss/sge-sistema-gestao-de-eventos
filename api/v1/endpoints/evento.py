...
from utils.errors.exception_docs import integrity_error, evento_not_found

router = APIRouter()

@router.post("/", response_model=EventoResponseSchema, status_code=status.HTTP_201_CREATED, responses=integrity_error) # Ver response model
...
@router.get("/{evento_id}", response_model=EventoResponseSchemaCompleto, status_code=status.HTTP_200_OK, responses=evento_not_found)
async def get(evento_id: int, db: AsyncSession = Depends(get_session)):
    return await get_evento(evento_id, db)

@router.put("/{evento_id}", response_model=EventoResponseSchema, status_code=status.HTTP_202_ACCEPTED, responses=evento_not_found)
...
@router.delete("/{evento_id}", status_code=status.HTTP_204_NO_CONTENT, responses=evento_not_found)
